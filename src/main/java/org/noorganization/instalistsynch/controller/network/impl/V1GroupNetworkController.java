...
public class V1GroupNetworkController implements IGroupNetworkController {
    ...
    public void createGroup(ICallbackCompleted<GroupInfo> _callbackCompleted) {
        Call<GroupInfo> groupCall =
                ApiUtils.getInstance().getUnauthorizedInstantListApiService().registerGroup();
        groupCall.enqueue(new UnauthorizedCallbackHandler<>(_callbackCompleted, groupCall));
        Log.i(LOG_TAG, "createGroup: enquedCallback");
    }

    @Override
    public void joinGroup(ICallbackCompleted<DeviceInfo> _callback, String _groupAccessToken,
            String _deviceName, int _groupId, String _secret) {
        ...
        Call<DeviceInfo> joinGroupCall =
                ApiUtils.getInstance().getUnauthorizedInstantListApiService()
                        .registerDevice(_groupId, registration);
    ...
    @Override
    public void deleteMemberOfGroup(IAuthorizedCallbackCompleted<Void> _callback, String _authToken,
            int _groupId, int _deviceId) {
        Call<Void> deleteCall =
                ApiUtils.getInstance().getAuthorizedApiService(IGroupApiService.class, _authToken)
                        .deleteDevicesOfGroup(_groupId, _deviceId);
    ...
    @Override
    public void requestGroupAccessToken(IAuthorizedCallbackCompleted<GroupInfo> _callback,
            int _groupId, String _authToken) {
        ...
        requestAccessTokenCall.enqueue(new AuthorizedCallbackHandler<>(_groupId,
                _callback,
                requestAccessTokenCall));
        Log.i(LOG_TAG, "requestGroupAccessToken: enquedCallback");
    }

    @Override
    public void getGroupMembers(IAuthorizedCallbackCompleted<List<DeviceInfo>> _callback,
            int _groupId, String _authToken) {
    ...
    @Override
    public void authorizeGroupMember(IAuthorizedCallbackCompleted<Void> _callback,
            GroupMember _groupMember, String _authToken) {
        if (_groupMember == null || _groupMember.hasNullFields()) {
            EventBus.getDefault()
                    .post(new ErrorMessageEvent(R.string.abc_error_member_not_existing));
            return;
        }
        DeviceInfo deviceInfo = new DeviceInfo();
        deviceInfo.setId(_groupMember.getDeviceId());
        deviceInfo.setName(_groupMember.getName());
        deviceInfo.setAuthorized(true);

        Call<Void> authorizeGroupMember = ApiUtils.getInstance()
                .getAuthorizedApiService(IGroupApiService.class, _authToken)
                .updateDeviceOfGroup(_groupMember.getGroupId(),
                        _groupMember.getDeviceId(),
                        deviceInfo);
        authorizeGroupMember.enqueue(new AuthorizedCallbackHandler<>(_groupMember.getGroupId(),
                _callback,
